...
設定画面の「メンテナンス」セクションに「重複画像をチェック」ボタンを追加しました。選択フォルダー内の画像をXXH3ハッシュで走査し、同一内容の画像をグループ化して件数を表示します。

メモリ節約のため、

- 1MB以下のファイルは一括読み込みで高速にハッシュ化
- 1MB超は256KBチャンクのストリーミングでハッシュ化
...
というエラーが出ます。
エラーメッセージにしたがって`/android/app/build.gradle.kts`にdependenciesを追加すれば動くっぽいのですが、今回のプロジェクトではエラーが消えなかったのでSelect TF Ops不使用版を生成するようにしています。
上のエラーの消し方がわかる方がいましたら教えていただきたいです。
